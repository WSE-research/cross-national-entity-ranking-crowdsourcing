...
my @suffixlist = (".txt");
my ($name,$path,$suffix) = fileparse($file_annotations,@suffixlist);
my $dirname  = dirname($file_annotations);

my $file_output = $dirname."/".$name.".scores";
